...
router.post('/launch', async (req: Request, res: Response): Promise<void> => {
    ...
    if (!token) { res.status(400).json({ error: 'Missing LTI token' }); return; }
    ...
    try {
        ...
        if (context.role === 'Instructor' && context.courseId) {
            syncRosterForCourse(context.courseId).catch(err =>
                console.warn('[NRPS] Background sync failed:', err.message)
            );
        }
 ...
 */
router.get('/exam/:title', async (req: Request, res: Response): Promise<void> => {
 ...
 */
router.patch('/bp/:courseId/:studentId', async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        const record = await BrowniePointModel.findOneAndUpdate(
            { studentId, courseId },
            {
 ...
 */
router.post('/bp/sync/:courseId', async (req: Request, res: Response): Promise<void> => {
    const { courseId } = req.params;
    try {
        const result = await syncRosterForCourse(courseId);
        ...
        for (const member of members) {
            ...
            if (!existing) {
                ...
                await BrowniePointModel.updateOne(
                    ...
                    { studentName: member.name, studentEmail: member.email, lastSyncedAt: new Date() }
                );
            }
        }

        console.log(`[NRPS] Synced course ${courseName} (${courseId}): ${members.length} members, ${newCount} new.`);
